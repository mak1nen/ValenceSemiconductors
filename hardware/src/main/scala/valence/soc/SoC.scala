...
class SoC(p: CoreParams) extends Module {
  ...
  // memory fill for ICache — simplified: use BootROM + SRAM
  ...
  core.io.dcache_rdata:= dcache.io.resp.rdata
  core.io.dcache_stall:= dcache.io.resp.stall

  // DCache fill — from SRAM
  dcache.io.mem.valid := false.B
  dcache.io.mem.data  := 0.U

  // CLINT
  clint.io.addr  := core.io.dcache_addr
  clint.io.wen   := core.io.dcache_wen
  clint.io.wdata := core.io.dcache_wdata
  clint.io.valid := core.io.dcache_valid &&
  ...
  uart.io.tx_valid := core.io.dcache_valid && core.io.dcache_wen &&
  ...
  sram.io.wen   := core.io.dcache_wen
  sram.io.wdata := core.io.dcache_wdata
  sram.io.wstrb := 0xFF.U
  sram.io.valid := core.io.dcache_valid &&
